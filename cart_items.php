...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (isset($_POST['remove'])) {
		...
		$RID=$_POST['remove'];
		$sql = "DELETE FROM `cart_items` WHERE `cart_items`.`product_id` = ".$RID;
...
<!---------Header----------->
	...
	<?php
		...
		$loop="SELECT * FROM `cart_items`";

		$value = $mysqli -> query($loop);

		if(mysqli_num_rows($value)>=1) {


			while($product = mysqli_fetch_array($value)){
				...
				if(mysqli_num_rows($result)==1){
					...
					$_SESSION['product_name'] = $row[1];
					$_SESSION['product_price'] = $row[2];
					$_SESSION['image'] = $row[3];

					echo "<section id=\"Single_product2\" class=\"Single_product\"><div class=\"small_container\"><div class=\"row\"><div class=\"col-md-3 text-center\" ><img src=\"",$_SESSION['image']," \" class=\"product_detail_img1\" width=\"100%\"></div><div class=\"col-md-3 offset-md-1 \"><h1>",$_SESSION['product_name'],"</h1><h3>$",$_SESSION['product_price'],"</h3><button type=\"submit\" form=\"product_form\" name=\"remove\" class=\"btn btn-primary btn-lg bg-info\" value=\"",$row[0],"\">Delete item</button></div></div></div></section>";
					}
				 }
			}
		}
		 else{
		 	echo "<section id=\"Single_product\"><div><h2> Cart is empty</h2></div></section>";
		 }
